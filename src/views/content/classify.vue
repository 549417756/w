  ...
  <div class="app-container">
    ...
    <data-table :list="list" :table-fields="tableFields">
      <template v-slot="{ row }">
        ...
        <svg-icon icon-class="delete" title="删除" @click.native="handleDelete(row, 'deleteContentClassify')" />
      </template>
    </data-table>
...
export default {
  ...
  methods: {
    getTableList() {
      this.$http.fetchContentClassifyList(this.listQuery).then(({ data: { lists, total }}) => {
        this.list = lists
        this.total = total
      })
    },
    ...
    createData() {
      if ('id' in this.ruleForm) {
        this.$http.editContentClassify(this.ruleForm).then(response => {
          this.message(response)
        })
      } else {
        this.$http.createContentClassify(this.ruleForm).then(response => {
          this.message(response)
        })
      }
    }
