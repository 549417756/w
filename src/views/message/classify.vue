  ...
  <div class="app-container">
    ...
    <data-table :list="list" :table-fields="tableFields">
      <template v-slot="{ row }">
        ...
        <svg-icon icon-class="delete" title="删除" @click.native="handleDelete(row, 'deleteMsgClassify')" />
      </template>
    </data-table>
...
export default {
  ...
  methods: {
    getTableList() {
      this.$http.fetchMsgClassifyList(this.listQuery).then(({ data: { lists, total }}) => {
        this.list = lists
        this.total = total
      })
    },
    ...
    createData() {
      if ('id' in this.ruleForm) {
        this.$http.editMsgClassify(this.ruleForm).then(response => {
          this.message(response)
        })
      } else {
        this.$http.createMsgClassify(this.ruleForm).then(response => {
          this.message(response)
        })
      }
    }
